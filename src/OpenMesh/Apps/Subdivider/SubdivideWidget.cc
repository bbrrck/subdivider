...
#include <OpenMesh/Tools/Subdivider/Uniform/QuadraticFittingT.hh>
    ...
    timer_(NULL), animate_step_(0), max_animate_steps_(4), msecs_(0)
{
  ...
  QFont bold_font("sans-serif", 10, QFont::Bold);
  ...
  viewer_widget_->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);
...
//   QButtonGroup *bgrp1 = new QButtonGroup( 1, QGroupBox::Vertical,"Subdivision Operators:");
  ...
  QRadioButton* radio0 = new QRadioButton( "QFR" );
  ...
  QRadioButton* radio6 = new QRadioButton( "Mod. Butterfly" );
  // QRadioButton* radio7 = new QRadioButton( "Catmull Clark" ); // Disabled, as it needs a quad mesh!
  
  radio0->setChecked( TRUE );
  
  sel_topo_type = SOP_QuadraticFitting;

  buttonGroup->addButton(radio0, SOP_QuadraticFitting);
  ...
  QLabel *scheme_label = new QLabel("Scheme");
  scheme_label->setFont( bold_font );
  grid->addWidget(scheme_label, 0, 0);

  grid->addWidget(radio0, 1, 0);
  // vbox->addWidget(radio1);
  // vbox->addWidget(radio2);
  grid->addWidget(radio3, 2, 0);
  grid->addWidget(radio4, 3, 0);
  // vbox->addWidget(radio5);
  grid->addWidget(radio6, 4, 0);
  // vbox->addWidget(radio7);

  QObject::connect( buttonGroup, SIGNAL( buttonPressed(int) ), this, SLOT( slot_select_sop(int) ) );

  QString mask();

	QLabel *weights_label = new QLabel("QFR Weights");
	weights_label->setFont( bold_font );
	weights_label->setAlignment(Qt::AlignCenter);
	grid->addWidget(weights_label, 0, 1);

	vWi = 1;
	grid->addWidget(new QLabel("vertex init"), 	1, 1);
	grid->addWidget(vW_init = new QLineEdit("   1.0000"),	1, 2);
					vW_init->setInputMask("0009.0000");
					vW_init->setMaximumWidth(100);
	QObject::connect( vW_init, SIGNAL( textChanged(QString) ), this, SLOT( slot_vWi_change(QString) ) );
	
	vWf = 0.1;
	grid->addWidget(new QLabel("vertex factor"), 2, 1);
	grid->addWidget(vW_fact = new QLineEdit("0.1000"),	2, 2);
					vW_fact->setInputMask("B.0000");
					vW_fact->setMaximumWidth(100);
	QObject::connect( vW_fact, SIGNAL( textChanged(QString) ), this, SLOT( slot_vWf_change(QString) ) );
	
	nWi = 0.001;
	grid->addWidget(new QLabel("normal init"), 	3, 1);
	grid->addWidget(nW_init = new QLineEdit("      0.0010"),	3, 2);
					nW_init->setInputMask("0000009.0000");
					nW_init->setMaximumWidth(100);
	QObject::connect( nW_init, SIGNAL( textChanged(QString) ), this, SLOT( slot_nWi_change(QString) ) );
	
	nWf = 0.01;
	grid->addWidget(new QLabel("normal factor"), 4, 1);
	grid->addWidget(nW_fact = new QLineEdit("0.0100"),	4, 2);
					nW_fact->setInputMask("B.0000");
					nW_fact->setMaximumWidth(100);
	QObject::connect( nW_fact, SIGNAL( textChanged(QString) ), this, SLOT( slot_nWf_change(QString) ) );



	QButtonGroup* buttonGroup_QFR = new QButtonGroup();

	buttonGroup_QFR->setExclusive( TRUE );

	QFR_vertexPick1 = new QRadioButton( "Projection" );
	QFR_vertexPick2 = new QRadioButton( "Foot Point" );

	QFR_vertexPick2->setChecked( TRUE );
	vertexPick = 2;

	buttonGroup_QFR->addButton(QFR_vertexPick1, 1);
	buttonGroup_QFR->addButton(QFR_vertexPick2, 2);

	QLabel *vertex_pick_label = new QLabel("QFR Vertex Picking");
	vertex_pick_label->setFont( bold_font );

	grid->addWidget(vertex_pick_label, 0, 3);
	grid->addWidget(QFR_vertexPick1, 1, 3);
	grid->addWidget(QFR_vertexPick2, 2, 3);

	QObject::connect( buttonGroup_QFR, SIGNAL( buttonPressed(int) ), this, SLOT( slot_select_vertexPick(int) ) );



	QLabel *normals_label = new QLabel("QFR Normals");
	normals_label->setFont( bold_font );
	nNN = true;
	grid->addWidget(normals_label, 3, 3);
	grid->addWidget(normalize_normals = new QCheckBox("normalize normals"), 4, 3);
					normalize_normals->setCheckState(Qt::Checked);
	QObject::connect( normalize_normals, SIGNAL( stateChanged(int) ), this, SLOT( slot_nNN_change(int) ) );
  ...
  subdivider_[SOP_QuadraticFitting]      	 = new Uniform::QuadraticFittingT<Mesh>;
...
void SubdivideWidget::slot_select_vertexPick(int method) {vertexPick = method; ChangeWeightsQFR();}

void SubdivideWidget::slot_vWi_change(QString text) {vWi = text.toDouble(); ChangeWeightsQFR();}
void SubdivideWidget::slot_vWf_change(QString text) {vWf = text.toDouble(); ChangeWeightsQFR();}
void SubdivideWidget::slot_nWi_change(QString text) {nWi = text.toDouble(); ChangeWeightsQFR();}
void SubdivideWidget::slot_nWf_change(QString text) {nWf = text.toDouble(); ChangeWeightsQFR();}
void SubdivideWidget::slot_nNN_change(int state) 	{nNN = (state == 2); 	ChangeWeightsQFR();}

void SubdivideWidget::ChangeWeightsQFR() {subdivider_[SOP_QuadraticFitting]->ChangeWeights(vWi,vWf,nWi,nWf,nNN,vertexPick);}
...
void SubdivideWidget::reset_slot()
{
  ...
  if (cur_topo_type != SOP_Undefined)
	subdivider_[cur_topo_type]->update_normals(viewer_widget_->mesh());
...
void SubdivideWidget::subdiv_slot()
{
  ...
  std::clog << "time elapsed: " << t.elapsed() << " ms" << std::endl;
...
SubdivideWidget::save_slot()
{
  ...
  if (!filename.isEmpty()){
    if ( OpenMesh::IO::write_mesh(viewer_widget_->mesh(), filename.toStdString()/*, OpenMesh::IO::Options::Binary*/) )
      std::cerr << "ok\n";
    else
      std::cerr << "FAILED\n";
  }
